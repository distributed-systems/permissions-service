(function() {
    ...
    module.exports = class PermissionInstance extends Hook {
         ...
         */
        isActionAllowed(service, resource, actionName, dontRecordFails = false) {
            ...
            if (this.allowedCache.has(cacheId) && this.allowedCache.get(cacheId)) return true;
                ...
                // know how is not getting what he wants
                if (!isAllowed && !dontRecordFails) this.reportDeniedAuthorization(service, resource, actionName);
        ...
        */
        reportDeniedAuthorization(service, resource, actionName) {
            ...
            if (debug) {
                log.warn(`Access denied for the ${actionName} action on ${service}/${resource}:`);

                for (const p of this.permissions) {
                    log.info(`Assigned roles: ${p.roles ? p.roles.map(r => `${r.identifier}(${r.id})`).join(', ') : ''}`);
                    log.info(`Subject: type=${p.subject.type}, subjectId=${p.subject.id}, internalSubjectId=${p.subject.internalId}${p.subject.data && p.subject.data.serviceName ? `, serviceName=${p.subject.data.serviceName}`: ''}${p.subject.data && p.subject.data.tenantIdentifier ? `, tenantIdentifier=${p.subject.data.tenantIdentifier}`: ''}`);
                }
        ...
        /**
        * returns an array of tokens and their roles
        */
        getTokenRoleIds() {
            const tokens = [];
            ...
            this.permissions.forEach((permission) => {
                tokens.push({
                    ...
                    , roles: permission.roles ? permission.roles.map(r => r.id) : []
                });
            });
